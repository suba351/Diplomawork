...
def subs_values():
    a, b, E, mu, D, k, k_rez, rho, h, p0, b_a = symbols('a b E mu D k k_rez rho h p0_2 b_a', real=True)
    ...
    M = Matrix(np.load(r"F:\NIR_4th_semestr\Plate\M_Matrix.npy", allow_pickle=True))
    C = Matrix(np.load(r"F:\NIR_4th_semestr\Plate\C_Matrix.npy", allow_pickle=True))
    ...
    return M, C, float(values[b] / values[a])
